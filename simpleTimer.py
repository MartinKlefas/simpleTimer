...
class Timer:
    ...
    def stop(self, event_name= None):
        if event_name is None:
            if self.event_stack:
                event_name = self.event_stack.pop()  # Pop the last event name from the stack
            else:
                raise ValueError("No event name provided and no event names in stack.")
        else:
            if event_name in self.event_stack:
                self.event_stack.remove(event_name)  # Remove the specified event name from the stack
            else:
